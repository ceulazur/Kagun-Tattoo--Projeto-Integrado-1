...
class Service {
  ...
  async buscarRegistroPorId(id) {
    try {
      ...
      throw new BadRequestError(
        `Erro ao buscar registro por ID: ${erro.message}`
      );
    }
  }
  ...
  async buscarRegistroPorCampo(where, extras = {}, throwError = true) {
    try {
      ...
      const registro = await prisma[this.model].findUnique({
        where,
        ...extras,
      });

      if (!registro && throwError)
        throw new NotFoundError("Registro não encontrado.");

      return registro;
    } catch (erro) {
      throw new BadRequestError(
        `Erro ao buscar registro em ${this.model}: ${erro.message}`
      );
    }
  }
  ...
  async buscarPrimeiroRegistroPorCampo(where, include = {}) {
    try {
      ...
      throw new BadRequestError(
        `Erro ao buscar primeiro registro: ${erro.message}`
      );
    }
  }
