...
class SessaoController extends Controller {
  ...
  async agendarSessao(req, res, next) {
    try {
      ...
      const sessao = await this.service.agendarSessao({
        idCliente,
        idTatuador,
        dataHorario,
      });
      ...
      return res
        .status(201)
        .json({ mensagem: "Sessão agendada com sucesso.", sessao });
    } catch (erro) {
      next(erro);
    }
  }
  ...
  async listarSessoes(req, res, next) {
    try {
      const sessoes = await this.service.listarSessoes(
        req.filtros,
        req.paginacao
      );
  ...
  async atualizarSessao(req, res, next) {
    try {
      ...
      return res
        .status(200)
        .json({ mensagem: "Sessão atualizada com sucesso.", sessaoAtualizada });
    } catch (erro) {
      next(erro);
    }
  }
  ...
  async cancelarSessao(req, res, next) {
    try {
      ...
      const sessaoCancelada = await this.service.cancelarSessao({
        idSessao: Number(id),
      });

      return res
        .status(200)
        .json({ mensagem: "Sessão cancelada com sucesso.", sessaoCancelada });
    } catch (erro) {
      next(erro);
    }
  }
